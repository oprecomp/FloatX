...
class CheckValidRepresentationOfFloatx5_10
    : public ::testing::TestWithParam<std::tuple<uint16_t, uint16_t>> {};

class CheckValidRepresentationOfFloatx4_8
    : public ::testing::TestWithParam<std::tuple<uint16_t, uint16_t>> {};
...
TEST_P(CheckValidRepresentationOfFloatx5_10, subnormal)
{
    ...
    for (int im = 0; im < nm; ++im) {
        ...
        test<flx::floatx<5, 10>>(d);
    }
}

TEST_P(CheckValidRepresentationOfFloatx5_10, regular)
{
    uint16_t const e = std::get<0>(GetParam());
    uint16_t const m = std::get<1>(GetParam());
...
TEST_P(CheckValidRepresentationOfFloatx4_8, subnormal)
{
    uint16_t const e = std::get<0>(GetParam());
    uint16_t const m = std::get<1>(GetParam());
    ...
    for (int im = 0; im < nm; ++im) {
        double d = h.iterateSubnormalRange(im);
        test<flx::floatx<4, 8>>(d);
    }
}

TEST_P(CheckValidRepresentationOfFloatx4_8, regular)
{
    uint16_t const e = std::get<0>(GetParam());
    uint16_t const m = std::get<1>(GetParam());
