...
class MyTest : public ::testing::TestWithParam<std::tuple<uint64_t, uint64_t>> {
};
...
TEST_P(MyTest, TestFormula)
{
    ...
    double d = flx::detail::reinterpret_bits_as<double>(in_number);
    double expected = flx::detail::reinterpret_bits_as<double>(out_number);

    // IEEE half.
    flx::floatx<5, 10> fx = d;
    double recoverd_fx = (double)(fx);
    ...
    uint64_t out = flx::detail::reinterpret_as_bits(recoverd_fx);
